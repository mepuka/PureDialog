...
const llmLayer = Layer.succeed(LLMService, {
  transcribeMedia: (jobId: Core.JobId) =>
    Effect.succeed({
      turns: [
...
describe("generateTranscript", () => {
  it.effect("produces transcript and completed job", () =>
    Effect.gen(function*() {
      const { jobId, processingJob } = makeProcessingJob()

      const { completedJob, transcript } = yield* generateTranscript(processingJob).pipe(
        Effect.provide(Layer.mergeAll(llmLayer, artifactStoreLayer))
      )
