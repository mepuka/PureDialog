  ...
  describe("createJob", () => {
    ...
    it.effect("should fail when configured to fail", () =>
      Effect.gen(function*() {
        ...
        const job = createQueuedJob()
        const result = yield* mockStore.createJob(job).pipe(Effect.exit)
  ...
  describe("findJobById", () => {
    ...
    it.effect("should fail when configured to fail", () =>
      Effect.gen(function*() {
        ...
        const result = yield* mockStore.findJobById("job_test").pipe(Effect.exit)
        assert.isTrue(result._tag === "Failure")
      }))
  })
  describe("findJobByIdempotencyKey", () => {
    ...
    it.effect("should fail when configured to fail", () =>
      Effect.gen(function*() {
        ...
        const result = yield* mockStore.findJobByIdempotencyKey("test-key").pipe(Effect.exit)
        assert.isTrue(result._tag === "Failure")
      }))
  })
  describe("updateJobStatus", () => {
    ...
    it.effect("should update job with transcript ID", () =>
      Effect.gen(function*() {
        const { mockStore } = createMockJobStore()
        const job = createProcessingJob()
        const transcriptId = "trn_test123"
        yield* mockStore.createJob(job)
    ...
    it.effect("should fail when job not found", () =>
      Effect.gen(function*() {
        const { mockStore } = createMockJobStore()
        const nonExistentJobId = "job_nonexistent"
        const result = yield* mockStore.updateJobStatus(nonExistentJobId, "Failed").pipe(Effect.exit)
        assert.isTrue(result._tag === "Failure")
      }))
    it.effect("should fail when configured to fail", () =>
      Effect.gen(function*() {
        ...
        const job = createQueuedJob()
        yield* mockStore.createJob(job)
        const result = yield* mockStore.updateJobStatus(job.id, "Processing").pipe(Effect.exit)
        assert.isTrue(result._tag === "Failure")
      }))
  })
  describe("integration scenarios", () => {
    ...
    it.effect("should handle concurrent operations correctly", () =>
      Effect.gen(function*() {
        const { capturedJobs, mockStore } = createMockJobStore()
        const jobs = [
          createQueuedJob({ id: "job_1", idempotencyKey: "key1:jobs:hash1" }),
          createQueuedJob({ id: "job_2", idempotencyKey: "key2:jobs:hash2" }),
          createQueuedJob({ id: "job_3", idempotencyKey: "key3:jobs:hash3" })
        ]
...
// # sourceMappingURL=JobStore.test.js.map
