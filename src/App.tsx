...
import type { Transcript, UsageMetadata, VideoJob, SpeakerConfig, TranscriptEntry } from './types';
 ...
 */
const extractLastCompleteObject = (jsonString: string): TranscriptEntry | null => {
    ...
    if (startBraceIndex !== -1) {
        ...
        try {
            ...
            if (parsed && typeof parsed.speaker === 'string' && typeof parsed.dialogue === 'string') {
...
const App: React.FC = () => {
  ...
  const [speakers, setSpeakers] = useState<SpeakerConfig[]>([
    { id: crypto.randomUUID(), name: 'Host', description: 'Typically introduces the show and directs the conversation.' },
    { id: crypto.randomUUID(), name: 'Guest', description: 'The person being interviewed or participating in the discussion.' },
  ]);
  ...
  const handleTranscribe = useCallback(async (videoUrl: string) => {
    ...
    setVideoJobs(prev => prev.map(job => 
      job.id === videoId ? { ...job, status: 'transcribing', error: undefined, streamingTimestamp: null, streamingSnippet: null } : job
    ));
    ...
    try {
      const { transcript: result, metadata } = await transcribeVideo(videoUrl, speakers, handleStreamChunk, controller.signal);
      setVideoJobs(prev => prev.map(job => 
        job.id === videoId ? { 
            ...job, 
            status: 'completed', 
      ...
      const isCancellation = err instanceof Error && err.message === 'Transcription was cancelled.';
      const errorMessage = isCancellation ? 'Cancelled by user.' : (err instanceof Error ? err.message : 'An unknown error occurred.');
       setVideoJobs(prev => prev.map(job => 
  ...
  const handleCancelTranscription = useCallback((videoId: string) => {
    ...
    if (!activeJob) {
      return (
        <div className="text-center text-gray-500">
          ...
          <p className="mt-2">Select a completed transcript, or start a new transcription.</p>
        </div>
      );
    }
    ...
    switch (activeJob.status) {
      case 'transcribing':
        return (
          <div className="w-full text-center flex flex-col items-center justify-center p-4">
            <h3 className="text-lg font-semibold text-white mb-4">Transcription in Progress...</h3>
      ...
      case 'completed':
        return activeJob.transcript ? (
          <TranscriptView transcript={activeJob.transcript} videoUrl={activeJob.url} metadata={activeJob.metadata || null} />
        ) : null;
      case 'failed':
        return (
           <div className="text-center text-red-400 p-4">
              ...
              <p className="mt-2 text-sm text-gray-400 max-w-md">{activeJob.error}</p>
           </div>
        );
      case 'pending':
         return (
            <div className="text-center text-gray-500">
              ...
              <p className="mt-2">Click "Transcribe" on a video in the queue to begin.</p>
            </div>
          );
      default:
        return null;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
          ...
          <div className="flex flex-col space-y-6">
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
              <label htmlFor="text-input" className="block text-lg font-medium text-white mb-2">
