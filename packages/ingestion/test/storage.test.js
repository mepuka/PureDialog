...
const TestCloudStorageService = Layer.sync(IngestionLayer.CloudStorageService, () => ({
    putObject: (bucket, key, _data) => Effect.gen(function* () {
        yield* Effect.logInfo(`Mock putObject: ${bucket}/${key}`);
        // Simulate successful operation
    }),
    getObject: (bucket, key, _schema) => Effect.gen(function* () {
        yield* Effect.logInfo(`Mock getObject: ${bucket}/${key}`);
        // Return none for simplicity
        return Option.none();
    }),
    deleteObject: (bucket, key) => Effect.gen(function* () {
        return yield* Effect.logInfo(`Mock deleteObject: ${bucket}/${key}`);
    }),
    listObjects: (bucket, prefix) => Effect.gen(function* () {
        yield* Effect.logInfo(`Mock listObjects: ${bucket}/${prefix}`);
        return [];
    }),
    objectExists: (bucket, key) => Effect.gen(function* () {
        yield* Effect.logInfo(`Mock objectExists: ${bucket}/${key}`);
        return false;
    })
}));
const TestEnvironment = Layer.mergeAll(TestCloudStorageConfig, TestCloudStorageService);
describe("CloudStorageService", () => {
    it.effect("should provide basic operations", () => Effect.gen(function* () {
    ...
    it.effect("should handle configuration", () => Effect.gen(function* () {
