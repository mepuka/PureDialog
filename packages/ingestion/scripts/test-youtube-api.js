...
const program = Effect.gen(function* () {
    ...
    const videoId = Option.fromNullable(cliArg).pipe(Option.map((value) => value), Option.getOrElse(() => defaultVideoId));
...
await Effect.runPromise(program.pipe(Effect.provide(Layer.mergeAll(YoutubeApiClientLive, YouTubeServiceLive)), Effect.catchAllCause((cause) => Console.error(Cause.pretty(cause)))));
