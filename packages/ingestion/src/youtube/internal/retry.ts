...
export const withRetry = (config: YoutubeConfigInterface) => <A, R>(effect: Effect.Effect<A, YoutubeApiError, R>) =>
  effect.pipe(Effect.retry(youtubeRetrySchedule(config.retryAttempts)));
